...
public:
  ...
  //Methodes for transport array
  void initializePersistentCommunicationsTransportArray(const int &size);
  void finalizePersistentCommunicationsTransportArray(const int &lvlMax);
  void communicationsTransportArray(Variable nameVector, const int &dim, int lvl, int num=0, int index=-1);
...
private:
  ...
  std::vector<double **> m_bufferReceiveTransportArray;
  std::vector<double **> m_bufferSendTransportArray;
  ...
  std::vector<MPI_Request **> m_reqSendTransportArray;
  std::vector<MPI_Request **> m_reqReceiveTransportArray;
