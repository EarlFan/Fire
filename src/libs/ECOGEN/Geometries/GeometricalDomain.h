...
public:
  ...
  // added by FanE
  //! \brief     Method for mass fraction in sharp interface
  //! \param     posElement        Point coordinates
  //! \param     lvl               Level of the cell
  //! \return    Return the mass fraction of the gas.
  virtual double alphaCell(Coord &posElement, const int &lvl) const { Errors::errorMessage("alphaCell not available for required GeometricalDomain"); };

  virtual Coord centerDisc()  const { Errors::errorMessage("GeometricalDomain::centerDisc() not available for required GeometricalDomain"); };

  virtual double getRadius()  const { Errors::errorMessage("GeometricalDomain::getRadius() not available for required GeometricalDomain"); };

  Mixture* getMixture() const {return m_mixture;};
  
  virtual double getHalfPosition() const { Errors::errorMessage("GeometricalDomain::getHalfPosition() not available for required GeometricalDomain"); };

  // for parallel initialization
  void fillIn2(Cell *cell, const int &numberPhases, const int &numberTransports) const ;

  // const std::string& modelName(Cell *cell) const { return cell->modelName(); };
