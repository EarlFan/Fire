...
void OutputXML::ecritDonneesPhysiquesXML(Mesh *mesh, std::vector<Cell *> *cellsLvl, std::ofstream &fileStream, bool parallel)
{
  ...
  //for (int phase = 0; phase < 1; phase++) //For reduced output
  {
    ...
    //Ecriture des variables scalars, Writing scalar variables
    for (int var = 1; var <= m_cellRef.getPhase(phase)->getNumberScalars(); var++) {
  ...
  //2) Ecriture des donnees mixture, Writing mixture data
  //-------------------------------
  if (m_run->m_numberPhases > 1)
  {
    int mixture = -1;
  ...
  //------------------------
  int gradRho = -4;
  fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"gradRho_Norm\"";
  if (!parallel) {
    fileStream << " format=\"" << format << "\">" << std::endl;
  ...
  //5.1) Ecriture gradient Y_0th
  //------------------------
  int grad_Y_0th = -4;//gradient Y[0th]
  fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"grad_Y_0th\" NumberOfComponents=\"3\"";
  if (!parallel) {
    fileStream << " format=\"" << format << "\">" << std::endl;
    mesh->recupereDonnees(cellsLvl, jeuDonnees, -1, grad_Y_0th);
    this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
    fileStream << std::endl;
    fileStream << "        </" << prefix << "DataArray>" << std::endl;
  }
  ...
  //5.2) Ecriture gradient U
  int grad_U = -2;
  for (int var = 1; var <= m_cellRef.getMixture()->getNumberVectors(); var++)
  {
    fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"gradU \" NumberOfComponents=\"3\"";
    if (!parallel) {
      fileStream << " format=\"" << format << "\">" << std::endl;
      mesh->recupereDonnees(cellsLvl, jeuDonnees, -var, grad_U);
      this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
      fileStream << std::endl;
      fileStream << "        </" << prefix << "DataArray>" << std::endl;
    }
  ...
  //5.3) Ecriture gradient V
  int grad_V = -3;
  for (int var = 1; var <= m_cellRef.getMixture()->getNumberVectors(); var++)
  {
    fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"gradV \" NumberOfComponents=\"3\"";
    if (!parallel) {
      fileStream << " format=\"" << format << "\">" << std::endl;
      mesh->recupereDonnees(cellsLvl, jeuDonnees, -var, grad_V);
      this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
      fileStream << std::endl;
      fileStream << "        </" << prefix << "DataArray>" << std::endl;
    }
  ...
  // //5.4) Ecriture gradient of 3rd species
  // //------------------------
  // int grad_Y_C3H6O = -5;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"grad_Y_3rd\" NumberOfComponents=\"3\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, -1, grad_Y_C3H6O);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.5) Ecriture induction time, a scalar
  // //------------------------
  // int inductionTime = -5;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"inductionTime\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, inductionTime);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.6) Ecriture gradient of induction time
  // //------------------------
  // int gradient_inductionTime = -6;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"grad_tau_ind\" NumberOfComponents=\"3\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, -1, gradient_inductionTime);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  //5.5) Ecriture Mach number, a scalar
  //------------------------
  int machNumber = -6;
  fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"Ma\"";
  if (!parallel) {
    fileStream << " format=\"" << format << "\">" << std::endl;
    mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, machNumber);
    this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
    fileStream << std::endl;
    fileStream << "        </" << prefix << "DataArray>" << std::endl;
  }
  ...
  // //5.6) Ecriture rho_H2O_rate, a scalar
  // //------------------------
  // int rho_H2O_rate_Number = -7;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_H2O_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_H2O_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  //5.7) Ecriture hrr, a scalar
  //------------------------
  int hrr = -8;
  fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"HRR\"";
  if (!parallel) {
    fileStream << " format=\"" << format << "\">" << std::endl;
    mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, hrr);
    this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
    fileStream << std::endl;
    fileStream << "        </" << prefix << "DataArray>" << std::endl;
  }
  ...
  // //5.8) Ecriture rho_H2_rate, a scalar
  // //------------------------
  // int rho_H2_rate_Number = -9;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_H2_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_H2_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.9) Ecriture rho_H_rate, a scalar
  // //------------------------
  // int rho_H_rate_Number = -10;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_H_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_H_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.10) Ecriture rho_OH_rate, a scalar
  // //------------------------
  // int rho_OH_rate_Number = -11;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_OH_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_OH_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.11) Ecriture rho_O_rate, a scalar
  // //------------------------
  // int rho_O_rate_Number = -12;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_O_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_O_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.12) Ecriture rho_HO2_rate, a scalar
  // //------------------------
  // int rho_HO2_rate_Number = -13;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_HO2_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_HO2_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.13) Ecriture rho_H2O2_rate, a scalar
  // //------------------------
  // int rho_H2O2_rate_Number = -14;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_H2O2_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_H2O2_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.14) Ecriture rho_O2_rate, a scalar
  // //------------------------
  // int rho_O2_rate_Number = -15;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_O2_rate\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_O2_rate_Number);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
  ...
  // //5.15) Ecriture gradient P
  // int grad_P = -7;
  // for (int var = 1; var <= m_cellRef.getMixture()->getNumberVectors(); var++)
  // {
  //   fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"gradP \" NumberOfComponents=\"3\"";
  //   if (!parallel) {
  //     fileStream << " format=\"" << format << "\">" << std::endl;
  //     mesh->recupereDonnees(cellsLvl, jeuDonnees, -var, grad_P);
  //     this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //     fileStream << std::endl;
  //     fileStream << "        </" << prefix << "DataArray>" << std::endl;
  //   }
  ...
  // //5.15) Ecriture rho_excitedOH, a scalar
  // //------------------------
  // int rho_excitedOH = -16;
  // fileStream << "        <" << prefix << "DataArray type=\"Float64\" Name=\"rho_exitedOH\"";
  // if (!parallel) {
  //   fileStream << " format=\"" << format << "\">" << std::endl;
  //   mesh->recupereDonnees(cellsLvl, jeuDonnees, 1, rho_excitedOH);
  //   this->ecritJeuDonnees(jeuDonnees, fileStream, DOUBLE);
  //   fileStream << std::endl;
  //   fileStream << "        </" << prefix << "DataArray>" << std::endl;
  // }
