...
void CellInterfaceO2::computeFlux(const int &numberPhases, const int &numberTransports, double &dtMax, Limiter &globalLimiter, Limiter &interfaceLimiter, Limiter &globalVolumeFractionLimiter, Limiter &interfaceVolumeFractionLimiter, Prim type)
{
  ...
  switch (type) {
  case vecPhases:
    if (m_cellLeft->getLvl() == m_cellRight->getLvl()) {       //CoefAMR = 1 pour les deux, for both side
      this->addFlux(numberPhases, numberTransports, 1.);       //Ajout du flux sur maille droite, Addition of flux on straight mesh 
      this->subtractFlux(numberPhases, numberTransports, 1.);  //Retrait du flux sur maille gauche, Withdrawal of flux on left mesh 
    }
    else if (m_cellLeft->getLvl() > m_cellRight->getLvl()) {   
      // CoefAMR = 1 pour la gauche et on n'ajoute rien sur la maille droite
      // for the left and nothing is added on the right stitch 
      this->subtractFlux(numberPhases, numberTransports, 1.);  //Retrait du flux sur maille gauche, Withdrawal of flux on left mesh 
    }
