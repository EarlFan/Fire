...
CellO2::CellO2() : Cell(), m_vecPhasesO2(0), m_mixtureO2(0), m_vecTransportsO2(0), m_consSauvegarde(0), m_consTransportsSauvegarde(0),m_consHalfTime(0) 
{
}

//***********************************************************************

CellO2::CellO2(int lvl) : Cell(lvl), m_vecPhasesO2(0), m_mixtureO2(0), m_vecTransportsO2(0), m_consSauvegarde(0), m_consTransportsSauvegarde(0),m_consHalfTime(0) {}
...
void CellO2::allocate(const int &numberPhases, const int &numberTransports, const std::vector<AddPhys*> &addPhys, Model *model)
{
  ...
  model->allocateCons(&m_consHalfTime, numberPhases);
  ...
  double &alphaCellAfterOppositeSide, double &alphaCell, double &alphaCellOtherInterfaceSide, double &epsInterface)
{
  ...
  //Loop on the cell interfaces for the determination of the slopes of each side of the cell
	//-----------------------------------------------------------------------------------------
  int phase0(0);
    ...
    // Calculation of the slope on the left and on the right of the cell (AMR) 
    // based on the reference one (cell interface on the left or on the right, unknown)
      ...
      if (m_cellInterfaces[b] == &cellInterfaceRef) {
        ...
        //Produit scalar des normals avec celle de reference
            ...
            //Face predominantly along X
              ...
              // on the side of cellInterfaceRef
  ...
  double &epsInterface)
{
  ...
  //Solution for multiD Cartesian (can be a rough outline for the NS, to see ...)
  ...
  //Loop on the cell interfaces for the determination of the slope side opposite the CL
  //------------------------------------------------------------------------------------
  for (unsigned int b = 0; b < m_cellInterfaces.size(); b++) {
    //Calcul de la slope a gauche et a droite de la cell (AMR) en se basant sur celle de reference (cell interface a gauche ou a droite, inconnu)
    // Calculation of the slope on the left and on the right of the cell (AMR) based on the reference one (cell interface on the left or on the right, unknown)
...
void CellO2::predictionOrdre2(const double &dt, const int &numberPhases, const int &numberTransports, Symmetry *symmetry)
{
  ...
  m_cons->multiply(dt, numberPhases);                       //On multiply m_cons (bilan des flux) par dt/2
  ...
  m_consHalfTime->setCons(m_cons, numberPhases);
}
...
void CellO2::getDataToReceiveAndRefine(std::vector<double> &dataToReceive, std::vector<int> &dataSplitToReceive, const int &lvl, Eos **eos, int &counter, int &counterSplit,
  const int &nbCellsY, const int &nbCellsZ, const std::vector<AddPhys*> &addPhys, Model *model)
{
  if (m_lvl == lvl) {
    for (int k = 0; k < m_numberPhases; k++) {
      ...
      if(model->whoAmI()=="FIRE")
      {
        counter-=m_vecPhasesO2[k]->numberOfTransmittedVariables();
        m_vecPhasesO2[k]->getBuffer(dataToReceive, counter, eos);
      }
    ...
    if(model->whoAmI()=="FIRE")
    {
      counter-=m_mixtureO2->numberOfTransmittedVariables();
      m_mixtureO2->getBuffer(dataToReceive, counter);
    }
