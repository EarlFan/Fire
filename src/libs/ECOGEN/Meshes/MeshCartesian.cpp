...
void MeshCartesian::initializeGeometrieMonoCpu(TypeMeshContainer<Cell *> &cells, TypeMeshContainer<CellInterface *> &cellInterfaces, std::string ordreCalcul)
{
  ...
  for (int i = 0; i < m_numberCellsCalcul; i++)
  {
    ...
    // V*y_cell for CYLINDRICAL case with y as radial axis 
    if(this->getSymType()=="CYLINDRICAL")volume=volume*m_posYj[iy];
    // printf("cell X = %le, Y = %le, volume = %le.\n",m_posXi[ix],m_posYj[iy],volume);
    ...
    for (iy = 0; iy < m_numberCellsY; iy++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if(this->getSymType()=="CYLINDRICAL")
          surface = m_dYj[iy] * m_dZk[iz]*m_posYj[iy];
        else
  ...
  for (ix = 0; ix < m_numberCellsX; ix++)
  {
    for (iy = 0; iy < m_numberCellsY - 1; iy++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        cells[iMailleD]->addCellInterface(cellInterfaces[iFace]);
        if(this->getSymType()=="CYLINDRICAL")
          surface = m_dXi[ix] * m_dZk[iz]*(m_posYj[iy] + 0.5*m_dYj[iy]);
        else
  ...
  if (m_numberCellsY != 1)
  {
    ...
    for (ix = 0; ix < m_numberCellsX; ix++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if(this->getSymType()=="CYLINDRICAL")
          surface = m_dXi[ix] * m_dZk[iz] * 0;//how to define this?
        else
          surface = m_dXi[ix] * m_dZk[iz];
...
void MeshCartesian::initializeGeometrieParallele(TypeMeshContainer<Cell *> &cells, TypeMeshContainer<Cell *> &cellsGhost, TypeMeshContainer<CellInterface *> &cellInterfaces, std::string ordreCalcul)
{
  ...
  //Declaration of the mesh table
  //---------------------------------
  //Decoupage parallele du domain geometrique
  //Parallel division of the geometric domain
  ...
  for (int i = 0; i < m_numberCellsCalcul; i++)
  {
    ...
    // V*y_cell for CYLINDRICAL case with y as radial axis 
    if(this->getSymType()=="CYLINDRICAL")volume=volume*m_posYj[m_offsetY + iy];
    cells[i]->getElement()->setVolume(volume);
  ...
  //numberFacesInternes : number de faces communes a deux mailles
  // ------------------------------------------------ ---------------------
  // Allocation of Cartesian geometrical data to cell interfaces
  // ------------------------------------------------ ---------------------
  // Determination of the number of faces according to compute 1D / 2D / 3D
  // m_numberFaces: total number of faces
  // m_numberFacesLimites: number of faces on the limits
  // numberFacesInternes: number of faces common to two meshes
  m_numberFacesTotal = 0;
  int m_numberFacesLimites(0);
  if (m_numberCellsX != 1)
  {
  ...
  for (ix = 0; ix < m_numberCellsX - 1; ix++)
  {
    for (iy = 0; iy < m_numberCellsY; iy++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if(this->getSymType()=="CYLINDRICAL")
          surface = m_dYj[m_offsetY + iy] * m_dZk[m_offsetZ + iz]* m_posYj[m_offsetY + iy];
        else
  ...
  for (ix = 0; ix < m_numberCellsX; ix++)
  {
    for (iy = 0; iy < m_numberCellsY - 1; iy++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if(this->getSymType()=="CYLINDRICAL")
          surface = m_dXi[m_offsetX + ix] * m_dZk[m_offsetZ + iz] * (m_posYj[m_offsetY + iy] + 0.5*m_dYj[m_offsetY + iy]);
        else
    ...
    for (iy = 0; iy < m_numberCellsY; iy++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if (m_numberCpuX > 1 && m_CpuCoordX > 0) {
          ...
          // V*y_cell for CYLINDRICAL case with y as radial axis 
          double y_cell = cells[iMailleG]->getElement()->getPosition().getY();
          if(this->getSymType()=="CYLINDRICAL"){
            cells[iMailleG]->getElement()->
              setVolume(m_dXi[m_offsetX - 1] * m_dYj[m_offsetY + iy] * m_dZk[m_offsetZ + iz] * y_cell );}
          else{
        ...
        double tempSurf;
        if(this->getSymType()=="CYLINDRICAL")
          tempSurf = m_dYj[m_offsetY + iy] * m_dZk[m_offsetZ + iz]* m_posYj[m_offsetY + iy];
        else
          tempSurf = m_dYj[m_offsetY + iy] * m_dZk[m_offsetZ + iz];
        m_faces[iFace]->initializeAutres(tempSurf, normal, tangent, binormal);
    ...
    for (iy = 0; iy < m_numberCellsY; iy++)
    {
      ...
      { 
        ...
        if (m_numberCpuX > 1 && m_CpuCoordX < m_numberCpuX - 1) {
          ...
          // V*y_cell for CYLINDRICAL case with y as radial axis 
          double y_cell = cells[iMailleD]->getElement()->getPosition().getY();
          if(this->getSymType()=="CYLINDRICAL"){
            cells[iMailleD]->getElement()->
              setVolume(m_dXi[m_offsetX + ix + 1] * m_dYj[m_offsetY + iy] * m_dZk[m_offsetZ + iz] * y_cell );}
          else 
    ...
    for (ix = 0; ix < m_numberCellsX; ix++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if (m_numberCpuY > 1 && m_CpuCoordY > 0) {
          ...
          if(this->getSymType()=="CYLINDRICAL"){
            cells[iMailleG]->getElement()->
              setVolume(m_dXi[m_offsetX + ix] * m_dYj[m_offsetY - 1] * m_dZk[m_offsetZ + iz] * y_cell );}
          else 
        ...
        double tempSurf, ySurf;
        ySurf = m_posYj[m_offsetY + iy] - 0.5*m_dYj[m_offsetY + iy];
        if(this->getSymType()=="CYLINDRICAL")
        {
          // if(ySurf<1e-20) ySurf = m_posYj[m_offsetY + iy] + 0.5*m_dYj[m_offsetY + iy]; 
          tempSurf = m_dXi[m_offsetX + ix] * m_dZk[m_offsetZ + iz]*ySurf;
        }
        else
          tempSurf = m_dXi[m_offsetX + ix] * m_dZk[m_offsetZ + iz];
    ...
    for (ix = 0; ix < m_numberCellsX; ix++)
    {
      for (iz = 0; iz < m_numberCellsZ; iz++)
      {
        ...
        if (m_numberCpuY > 1 && m_CpuCoordY < m_numberCpuY - 1) {
          ...
          if(this->getSymType()=="CYLINDRICAL"){
            cells[iMailleD]->getElement()->
              setVolume(m_dXi[m_offsetX + ix] * m_dYj[m_offsetY + iy + 1] * m_dZk[m_offsetZ + iz]* y_cell );}
          else 
...
void MeshCartesian::decoupageParallele(std::string ordreCalcul, TypeMeshContainer<Cell *> &cells)
{
  ...
  if (m_geometrie == 1) {
    ...
    //Determination of the element number to send / receive
    numberElements = m_numberCellsY*m_numberCellsZ;

    //Number de mailles total en comptant les mailles necessaires au parallele;
    //Total number of meshes by counting the meshes necessary for the parallel;
    ...
    // The number of parallel meshes is located outside the number of "true" meshes
    // so starts at numberCells
    compteMaillesParallele = m_numberCellsCalcul;
    countElements = 0;
    if (rankCpu > 0)
    {
    ...
    m_numberCellsZ = m_numberCellsZGlobal;
    if (m_CpuCoordX < remainingCellsX) { ++m_numberCellsX; }// allocate the remainingCellX to the core with lower CpuCoodX
    if (m_CpuCoordY < remainingCellsY) { ++m_numberCellsY; }

    //Determination of the offset, what's offset? - fane
    ...
    for (int j = 0; j < m_numberCellsY; j++) {
      for (int i = 0; i < m_numberCellsX; i++) {
        ...
        if (var > 0) { //On veut recuperer les donnees scalars
          ...
          else if (phase == -5) { jeuDonnees.push_back(cellsLvl[0][numCell]->getMixture()->getInductionTime()); }
          else if (phase == -6) { jeuDonnees.push_back(cellsLvl[0][numCell]->getMixture()->getMach()); }
          else if (phase == -7) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[5]); }//density change rate of H2O
          else if (phase == -8) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getHfChangeRate()); }//enthalpy change rate
          else if (phase == -9) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[1]); }//density change rate of H2
          else if (phase == -10) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[2]); }//density change rate of H
          else if (phase == -11) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[3]); }//density change rate of OH
          else if (phase == -12) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[4]); }//density change rate of O
          else if (phase == -13) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[6]); }//density change rate of HO2
          else if (phase == -14) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[7]); }//density change rate of H2O2
          else if (phase == -15) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getDensityChangeRateArray()[9]); }//density change rate of O2
          else if (phase == -16) { jeuDonnees.push_back(cellsLvl[0][i]->getMixture()->getRhoExcitedOH()); }//numerical distribution of OH*
          // else if (phase == -7) { jeuDonnees.push_back(cellsLvl[0][numCell]->getGradientYI(0)); }// get the gradient of Y_SF6
          ...
          else if (phase == -2) { //gradient U
            Coord temp = cellsLvl[0][numCell]->getGradientU();
            jeuDonnees.push_back(temp.getX());
            jeuDonnees.push_back(temp.getY());
            jeuDonnees.push_back(temp.getZ());
          }
          else if (phase == -3) { //gradient V
            Coord temp = cellsLvl[0][numCell]->getGradientV();
            jeuDonnees.push_back(temp.getX());
            jeuDonnees.push_back(temp.getY());
            jeuDonnees.push_back(temp.getZ());
          }
          else if (phase == -4) { //gradient Y[0th]
            Coord temp = cellsLvl[0][numCell]->getGradientYI(0);
            jeuDonnees.push_back(temp.getX());
            jeuDonnees.push_back(temp.getY());
            jeuDonnees.push_back(temp.getZ());
          }
          else if (phase == -5) { //gradient Y[3rd]
            Coord temp = cellsLvl[0][numCell]->getGradientYI(3);
            jeuDonnees.push_back(temp.getX());
            jeuDonnees.push_back(temp.getY());
            jeuDonnees.push_back(temp.getZ());
          }
          else if (phase == -6) { //gradient induction time
            Coord temp = cellsLvl[0][numCell]->getGradientInductionTime();
            jeuDonnees.push_back(temp.getX());
            jeuDonnees.push_back(temp.getY());
            jeuDonnees.push_back(temp.getZ());
          }
