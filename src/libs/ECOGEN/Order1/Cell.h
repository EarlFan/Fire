...
class Cell
{
    public:
        ...
        virtual void addHalfTimeCons(const int &numberPhases) {Errors::errorMessage("methode addHalfTimeCons non dispo pour cell"); };

        //For additional physics
        //----------------------
        void prepareAddPhys();
        double selectScalar(Variable nameVariable, int num = 0) const;
        double selectMassi(int i) const;
        ...
        CellInterface* getCellInterface(int &b);
        ...
        Coord getGradientYI(int i);
        Coord getGradientU();
        Coord getGradientV();
        Coord getGradientW();
        Coord getGradientInductionTime();
        Coord getGradientP();
        Model *getModel();
        Coord& getVelocity();
        ...
        void chooseUnrefine(const double &xiJoin, int &nbCellsTotalAMR, const int &max_lvl); /*!< Choice for unrefinement of parent cell */
        void refineCellAndCellInterfaces(const int &nbCellsY, const int &nbCellsZ, 
          const std::vector<AddPhys*> &addPhys, Model *model, const bool &refineExternalCellInterfaces);           /*!< Refine parent cell by creation of children cells */
        ...
        virtual void getDataToReceiveAndRefine(std::vector<double> &dataToReceive, std::vector<int> &dataSplitToReceive, const int &lvl, Eos **eos, int &counter, int &counterSplit,
            const int &nbCellsY, const int &nbCellsZ, const std::vector<AddPhys*> &addPhys, Model *model);//-fane
        ...
        // added by Fan
        std::array<double,NS+2> const & getTransportProperties() const;
        void setTransportProperties(const std::array<double,NS+2> transArray);
        const std::string& modelName() const;
        // void fillFire(std::vector<GeometricalDomain*> &domains, const int &lvlMax, 
        //   Mixture* mixLP, Mixture* mixShock, Mixture * mixBubble, 
        //   double shock, Coord center, double radius);
        const std::string getSymType()const;
        double calcTauC();
        double calcTauR();
        double calcTauGradRho(); // AMR criterion based on grad(rho)

    protected:
      int m_numberPhases;
      int m_numberTransports;
      std::array<double,NS+2> m_transportProperties; 
      Phase **m_vecPhases;
      Mixture *m_mixture;
